...
class QKBroker(with_metaclass(MetaQKBroker, BrokerBase)):
    ...
    params = (
        ...
        ('ClientCodeTerminal', ''),  # Номер терминала # когда Код Клиента не соответствует Номеру Терминала
        ('FirmId', 'SPBFUT'),  # Фирма
    ...
    def buy(self, owner, data, size, price=None, plimit=None, exectype=None, valid=None, tradeid=0, oco=None, trailamount=None, trailpercent=None, parent=None, transmit=True, **kwargs):
        """Заявка на покупку"""
        if not self.p.ClientCodeTerminal: self.p.ClientCodeTerminal = self.p.ClientCode
        order = self.CreateOrder(owner, data, size, price, plimit, exectype, valid, oco, parent, transmit, True, ClientCode=self.p.ClientCodeTerminal, TradeAccountId=self.p.TradeAccountId, **kwargs)
    ...
    def sell(self, owner, data, size, price=None, plimit=None, exectype=None, valid=None, tradeid=0, oco=None, trailamount=None, trailpercent=None, parent=None, transmit=True, **kwargs):
        """Заявка на продажу"""
        if not self.p.ClientCodeTerminal: self.p.ClientCodeTerminal = self.p.ClientCode
        order = self.CreateOrder(owner, data, size, price, plimit, exectype, valid, oco, parent, transmit, False, ClientCode=self.p.ClientCodeTerminal, TradeAccountId=self.p.TradeAccountId, **kwargs)
        self.notifs.append(order.clone())  # Удедомляем брокера об отправке новой заявки на рынок
        return order
